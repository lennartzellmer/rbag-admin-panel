...
Run the setup script to bootstrap everything, including Docker services and the Zitadel Terraform outputs:

```bash
# From the project root
./dev_docker/setup-dev.sh
```
...
The setup script combines the manual steps into a single command. It will:

- Create `.env` from `.env.example` if missing
- Start Docker services using the `dev_docker/docker-compose.yml` file
- Run `terraform init` and `terraform apply` in `dev_docker/terraform`
- Capture `terraform output -json` and write the Zitadel values into `.env`
